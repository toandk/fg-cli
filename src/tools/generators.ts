...
import { command, heading, FGHeading, p, warning } from "./pretty"

// eslint-disable-next-line @typescript-eslint/no-unused-vars
export function showGeneratorHelp(toolbox: GluegunToolbox) {
  const inFG = isFGProject()
  const generators = inFG ? installedGenerators() : []

  FGHeading()
  heading("Fluter Generators")
  p()
  p("When you create a new app with FG CLI, it will install several generator")
  p("templates in the project folder under the `FG/templates` folder.")
  p()
  heading("Commands")
  p()
  command("--list  ", "List installed generators", ["FG g --list"])
  command(
    "--update",
    "Update installed generators. You can also use the 'FG update X' format",
    ["FG g --update", `FG g model --update`, `FG update model`, `FG update --all`],
  )
  ...
  if (inFG) {
    ...
    generators.forEach((g) => {
      command(g.padEnd(longestGen), `generates a ${g}`, [`FG g ${g} Demo`])
    })
  } else {
    warning("⚠️  Not in an FG project root. Go to your FG project root to see generators.")
  }
}
...
export function isFGProject(): boolean {
  return filesystem.exists("./FG") === "dir"
}

function FGDir() {
  const cwd = process.cwd()
  return filesystem.path(cwd, "FG")
}

function appDir() {
  const cwd = process.cwd()
  return filesystem.path(cwd, "lib")
}

function templatesDir() {
  return filesystem.path(FGDir(), "templates")
}
...
export function generateFromTemplate(generator: string, options: GeneratorOptions): string[] {
  ...
  const pascalCaseName = pascalCase(options.name).split("-").join("_")
  const kebabCaseName = kebabCase(options.name).split("-").join("_")
  const camelCaseName = camelCase(options.name).split("-").join("_")
...
/**
 * Directory where we can find FG CLI generator templates
 */
function sourceDirectory(): string {
  return filesystem.path(__filename, "..", "..", "..", "boilerplate", "FG", "templates")
}

/**
 * Finds generator templates in FG CLI
 */
...
/**
 * Copies over generators (specific generators, or all) from FG CLI to the project
 * FG/templates folder.
 */
export function installGenerators(generators: string[]): string[] {
  ...
  const targetDir = path(cwd(), "FG", "templates")

  if (!isFGProject()) {
    throw new Error("Not in an FG root directory (can't find ./FG folder)")
  }

  // for each generator type, copy it over to the FG/templates folder
